...
function RegionFlag({ code }: { code: string }) {
  ...
  if (normalizedCode === "PL") {
    return (
      <svg
        viewBox="0 0 16 12"
        className="h-3 w-4 rounded-[2px] shadow-sm"
        aria-hidden="true"
      >
  ...
  if (normalizedCode === "CA") {
    return (
      ...
      >
        ...
        <path
          d="M8 2.1 8.6 4 10 3.3 9.3 5.1 10.8 5.7 9.1 6.2 9.6 8 8.3 7.1 8 9.6 7.7 7.1 6.4 8 6.9 6.2 5.2 5.7 6.7 5.1 6 3.3 7.4 4 8 2.1Z"
          fill="#d52b1e"
        />
        <rect
          width="16"
  ...
  return (
    <span className="inline-flex h-3 min-w-4 items-center justify-center rounded-[2px] bg-slate-200 px-1 text-[7px] font-black leading-none text-slate-700 shadow-sm">
...
export function Dashboard({ onNavigate, employeeContext }: DashboardProps) {
  ...
  const handleQuickAsk = async () => {
    ...
    try {
      ...
      console.error(err);
      setQuickAnswer(
        "I'm sorry, I couldn't get an answer right now. Please try again or visit the full chat.",
      );
    } finally {
    ...
    fetch(`/api/feed?language=${effectiveLanguage}`)
      ...
      .then((data) => {
        ...
        const filtered = data.filter(
          (item: FeedItem) =>
            item.region === region || item.region === "Global",
        );
  ...
  const handleSummarizeFeed = async () => {
    ...
    try {
      ...
      const fullText =
        data.content ||
        "I couldn't generate a news brief. Please try again later.";
      let currentText = "";
      const words = fullText.split(" ");
      for (let i = 0; i < words.length; i++) {
        currentText += (i === 0 ? "" : " ") + words[i];
      ...
      console.error(err);
      setFeedSummary(
        "Local hub news update: Warsaw expansion planning is on track for Q3.",
      );
    } finally {
  ...
  const handleSummarizeMeetings = async () => {
    ...
    try {
      ...
      const fullText =
        data.content ||
        "I couldn't summarize the meetings. Please try again later.";
      let currentText = "";
      const words = fullText.split(" ");
      for (let i = 0; i < words.length; i++) {
        currentText += (i === 0 ? "" : " ") + words[i];
      ...
      console.error(err);
      setMeetingSummary(
        "Regional engineering integration metrics are improving weekly.",
      );
    } finally {
  ...
  const shortcuts = [
    {
      ...
      icon: "🌴",
      action: () =>
        onNavigate("chat", `How do I request PTO for region ${region}?`),
    },
    {
      label: "File Expense",
      icon: "💰",
      action: () =>
        onNavigate("chat", `What are the expense rules for ${region}?`),
    },
  ...
  const getDynamicPersonaCards = () => {
    switch (activePersonaId) {
      case "anna":
        return {
          card1Val: "26 Days",
          card1Lbl: "Accrued Annual PL Leave",
          card1Query: "What is my vacation balance?",
          card2Val: "PIT-11 Ready",
          card2Lbl: "Warsaw Tax Portal File",
          card2Query: "How do I download my PIT-11 document?",
          card3Val: "+2 Days",
          card3Lbl: "Paid Corporate Bridge Days",
          card3Query: "Tell me about Poland wellness bridge days",
          card4Val: "Platform Eng",
          card4Lbl: "Warsaw Spire Hub",
          card4Query: "Are there any internal platform engineering openings?",
        };
      case "oleksandr":
        return {
          card1Val: "24 Days",
          card1Lbl: "UA Annual Leave Statutory",
          card1Query: "How many vacation days do I accrue in Ukraine?",
          card2Val: "FOP Group III",
          card2Lbl: "Unified 5% Tax Filing Auto",
          card2Query: "How are Private Entrepreneur FOP taxes filed in UA?",
          card3Val: "Martial Law Update",
          card3Lbl: "Statutory holidays working days",
          card3Query: "What are the local holidays in Ukraine?",
          card4Val: "Software Dev",
          card4Lbl: "Kyiv Hotdesk Facility Available",
          card4Query: "Who is my regional HR contact in Ukraine?",
        };
      case "jane":
        return {
          card1Val: "Unlimited FTO",
          card1Lbl: "US Salaried Flexible Time Off",
          card1Query: "What is the US FTO policy?",
          card2Val: "W-4 Form Ready",
          card2Lbl: "Continuous Tax Withholding",
          card2Query: "How do I update W-4 withholdings in Workday?",
          card3Val: "Ends in 5 days",
          card3Lbl: "Health Benefits Open Enrollment",
          card3Query: "How do I select my health benefits for open enrollment?",
          card4Val: "Senior Analyst",
          card4Lbl: "Remote US East Desk",
          card4Query: "Who is my HR contact in USA?",
        };
      case "sophie":
        return {
          card1Val: "15-20 Days",
          card1Lbl: "Provincial Statutory Minimum",
          card1Query: "What are Ontario and Canada vacation guidelines?",
          card2Val: "T4 Slips Inbound",
          card2Lbl: "Canadian Tax & Remuneration Slip",
          card2Query: "Where can I log my Canadian T4 tax documents?",
          card3Val: "4% Corporate Match",
          card3Lbl: "Registered Retirement RRSP Match",
          card3Query: "Tell me about the Canadian RRSP Matching Policy",
          card4Val: "Solutions Architect",
          card4Lbl: "Toronto Flex Desk Space",
          card4Query: "Who is the HR coordinator for Exadel Canada?",
        };
      default:
        return {
          card1Val: "12 Days",
          card1Lbl: "Accrued Remaining",
          card1Query: "How do I file vacation?",
          card2Val: "1 Overdue",
          card2Lbl: "Compliance Course",
          card2Query: "Are there overdue compliance runs?",
          card3Val: "Ends in 5d",
          card3Lbl: "Open Election",
          card3Query: "Where are benefits listed?",
          card4Val: "Platform Engineer",
          card4Lbl: "Remote PL • Open",
          card4Query: "Open jobs listed",
        };
    }
  };
      ...
      <div className="flex-1 p-10 overflow-y-auto space-y-12">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm relative overflow-hidden transition-all hover:shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10 items-center">
            {/* Stat 1: Accrued PTO */}
            <div
              className="flex items-center gap-4 p-4 bg-rose-50/50 rounded-2xl border border-rose-100/40 hover:bg-rose-50 hover:scale-[1.01] transition-all cursor-pointer"
              onClick={() => onNavigate("chat", dynCards.card1Query)}
            >
              <div className="w-12 h-12 bg-rose-100/60 rounded-xl flex items-center justify-center text-rose-600 text-2xl shadow-inner">
                🌴
              </div>
              <div className="min-w-0">
                <div
            ...
            <div
              className="flex items-center gap-4 p-4 bg-blue-50/50 rounded-2xl border border-blue-100/40 hover:bg-blue-50 hover:scale-[1.01] transition-all cursor-pointer"
              onClick={() => onNavigate("chat", dynCards.card2Query)}
            >
              <div className="w-12 h-12 bg-blue-100/60 rounded-xl flex items-center justify-center text-blue-600 text-2xl shadow-inner">
                📂
              </div>
              <div className="min-w-0">
                <div className="text-xl font-black text-blue-800 tracking-tight leading-none mb-1 truncate">
                  {dynCards.card2Val}
                </div>
                <div className="text-[10px] font-black uppercase tracking-wider text-blue-600/70 truncate">
                  {dynCards.card2Lbl}
                </div>
              </div>
            </div>
            ...
            <div
              className="flex items-center gap-4 p-4 bg-purple-50/50 rounded-2xl border border-purple-100/40 hover:bg-purple-50 hover:scale-[1.01] transition-all cursor-pointer"
              onClick={() => onNavigate("chat", dynCards.card3Query)}
            >
              <div className="w-12 h-12 bg-purple-100/60 rounded-xl flex items-center justify-center text-purple-600 text-2xl shadow-inner">
                🎁
              </div>
              <div className="min-w-0">
                <div className="text-lg font-black text-purple-800 tracking-tight leading-none mb-1 truncate">
                  {dynCards.card3Val}
                </div>
                <div className="text-[10px] font-black uppercase tracking-wider text-purple-600/70 truncate">
                  {dynCards.card3Lbl}
                </div>
              </div>
            </div>
            ...
            <div
              className="flex items-center gap-4 p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100/40 hover:bg-emerald-50 hover:scale-[1.01] transition-all cursor-pointer"
              onClick={() => onNavigate("chat", dynCards.card4Query)}
            >
              <div className="w-12 h-12 bg-emerald-400 text-white rounded-xl flex items-center justify-center text-2xl shadow-inner">
                🏢
              </div>
              <div className="min-w-0">
                <div className="text-sm font-black text-slate-800 truncate leading-tight">
                  {dynCards.card4Val}
                </div>
                <div className="text-[10px] font-bold text-emerald-600 truncate mt-0.5">
                  {dynCards.card4Lbl}
                </div>
            ...
            <div className="space-y-3">
              {feedSummary && (
                ...
                >
                  ...
                  <button
                    onClick={() => setFeedSummary(null)}
                    className="p-1.5 text-primary-dark/40 hover:text-primary-dark"
                  >
                    <Zap size={14} />
                  </button>
                </motion.div>
              ...
              {meetingSummary && (
                ...
                >
                  ...
                  <button
                    ...
                    className="p-1.5 text-emerald-400 hover:text-emerald-600"
                  >
                    <Zap size={14} />
                  </button>
                </motion.div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          ...
                          className="text-[#075E54]/40 hover:text-[#075E54]"
                        >
                          <Zap size={12} />
                        </button>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {quickAnswer.toLowerCase().includes("knowledge") && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[9px] font-bold text-slate-700 uppercase tracking-wider hover:bg-slate-50 transition-all"
                          >
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-widest text-primary mb-0.5">
                Self-Service Hub
              </h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <button
              onClick={() =>
                onNavigate(
                  "chat",
                  `I'd like to request PTO for region ${region}`,
                )
              }
              className="bg-[#E6F2FF]/60 p-5 rounded-2xl text-left hover:bg-[#E6F2FF] transition-all border border-[#E6F2FF]/80"
            >
              <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-xl mb-4 shadow-sm">
                🌴
              </div>
              <h4 className="font-extrabold text-[#1F1F1F] text-sm">
                {t("requestPto")}
              </h4>
              <p className="text-[10px] text-slate-500 font-semibold opacity-80 mt-0.5">
                {t("ptoBalance")}
              </p>
            </button>
            ...
            <button
              onClick={() =>
                onNavigate("chat", "How do I book a desk in my office?")
              }
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-widest text-[#1F1F1F]/40 mb-1">
                {t("corporateArchives")}
              </h3>
              <h2 className="text-xl font-bold text-[#1F1F1F]">
                {t("exploreDirectories")}
              </h2>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 pb-12">
            {categories.map((cat) => (
              ...
              >
                <div
                  className={`w-10 h-10 ${cat.color || "bg-slate-100"} rounded-lg flex items-center justify-center text-xl group-hover:scale-105 transition-transform flex-shrink-0`}
                >
                  {cat.icon === "tickets" ? "🎫" : cat.icon}
                </div>
                <div className="min-w-0">
                  <h3 className="font-extrabold text-sm text-[#1F1F1F] truncate">
                    {cat.label}
                  </h3>
                  <p className="text-[10px] text-slate-405 font-bold truncate">
                    {cat.desc}
                  </p>
                </div>
              </button>
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              ...
              {recentFeed.map((item, idx) => (
                <motion.div
                  ...
                  className={`p-5 rounded-[24px] shadow-sm flex flex-col gap-1 border-l-[6px] hover:scale-[1.02] transition-transform cursor-default ${
                    idx % 2 === 0
                      ? "bg-[#F8F9FF] border-[#4D77FF]"
                      : "bg-[#F2FFFA] border-[#20C997]"
                  }`}
                >
                  <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-2">
                    {item.type}
                  </p>
                  <h4 className="font-black text-sm text-[#1F1F1F] leading-snug">
                    {item.title}
                  </h4>
                  <div className="text-[10px] text-slate-500 font-bold mt-1 tracking-tight italic opacity-60">
                    Verified Post • Just now
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="bg-peach rounded-[32px] p-6 shadow-sm border border-peach-light relative overflow-hidden group">
              ...
              <div className="bg-[#FFB300] text-white text-[9px] font-black uppercase px-2 py-0.5 rounded-md w-fit mb-3 relative z-10">
                Updated
              </div>
              <h4 className="font-black text-md text-[#1F1F1F] mb-2 leading-tight relative z-10">
                New Joiner Guide
              </h4>
              <p className="text-[11px] text-slate-600 font-medium leading-relaxed mb-4 relative z-10 italic">
                "Complete your first 30 days of onboarding through the
                interactive guide on your dashboard..."
              </p>
              <button
                onClick={() => onNavigate("knowledge")}
